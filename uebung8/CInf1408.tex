...
\begin{center}
  \begin{tikzpicture}[node distance=4cm]
    ...
    \path[->] % inlet
              (A) edge[loop above] node[above] {000/0, 001/0, 011/0} (A) % valid
              (A) edge[loop left]  node[left]  {010/1, 100/1, 101/1, 110/1} (A) % error
              (A) edge             node[above] {111/1} (B) % next
  
              % hold it
              (B) edge[loop above] node[above] {000/0, 001/0, 011/1}(B) % valid
              (B) edge[loop right] node[right] {010/1, 100/1, 101/1, 110/1}(B) % error
              (B) edge             node[right] {111/1}(C) % next
              
              % outlet
              (C) edge[loop below] node[below] {001/1, 011/1, 111/1}(C) % valid
              (C) edge[loop right] node[right] {010/1, 100/1, 101/1, 110/1}(C) % error
              (C) edge             node[below] {000/0}(D) % next
              
              % hold it
              (D) edge[loop below] node[below] {001/0, 011/1, 111/0}(D) % valid
              (D) edge[loop left]  node[left]  {010/1, 100/1, 101/1, 110/1}(D) % error
              (D) edge             node[left] {000/0}(A) % next
              ;
...
\begin{center}
  \begin{tikzpicture}
    \node[scale=.8,draw,rounded corners=5pt,fill=lightgray!50,text width=4cm] (legend) {
       ...
       An Übergängen: \makebox[4cm]{ $X_tX_mX_b/Y$ }
    };
  \end{tikzpicture}
\end{center}
...
Die beiden Kontrollsignale sind jeweils \texttt{goto BEGIN}.

\textbf{Kriterien:}

Das einzige Kriterium ist \texttt{VALID=1}.
