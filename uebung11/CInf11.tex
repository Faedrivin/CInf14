...
\node[block, below=of init, text width=\blockW] (loophead) 
    {\val\ \la (X+)};
\node[decision, below=of loophead, text width=\decW, shape aspect=2.5] (odd)
    {odd(\val)?};
\node[block, below=of odd, text width=\blockW] (push) 
    {PUSH \val \\ \stack\ \la \stack + 1};
\node[decision, right=of odd, text width=\decW, shape aspect=2.5] (255)
    {\val\ == 255?};
\node[decision, right=of odd, above=of 255, text width=\decW, shape aspect=2.5] (30)
    {\ptrl\ == 30?};
\node[block, right=of 30, text width=\blockW] (reset)
    {\ptrl\ \la low(\#0x200) \\ \ptrh\ \la high(\#0x200)};
\node[decision, above=of reset, text width=\decW, shape aspect=2.5] (done)
    {\stack\ == 0?};
