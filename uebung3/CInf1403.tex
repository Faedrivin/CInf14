...
\begin{tikzpicture}
   \matrix (m)[
      nodes={draw,shape=rectangle,text width=3em,minimum height=3em,text
      centered},
      matrix of nodes,
      clip,
      inner sep=0pt,
   ...
   \draw[highlight,draw=green]($(m-1-1.south west) + (3pt,3pt)$) rectangle ($(m-1-1.north east) + (-3pt,2cm)$);
   \draw[highlight,draw=green]($(m-4-1.north east) + (-3pt,-3pt)$) rectangle ($(m-4-1.south west) + (3pt,-2cm)$);
   \draw[highlight,draw=green]($(m-1-4.south west) + (3pt,3pt)$) rectangle ($(m-1-4.north east) + (-3pt,2cm)$);
   \draw[highlight,draw=green]($(m-4-4.north east) + (-3pt,-3pt)$) rectangle ($(m-4-4.south west) + (3pt,-2cm)$);
   \draw[highlight,loosely dashed,draw=red]($(m-4-3.north west) + (3pt,-3pt)$) rectangle ($(m-4-4.south east) + (-3pt,--3pt)$);
\end{tikzpicture}
\end{center}

Die DMF ist daher $\comp a \comp b \comp c + \comp b c \comp d + abc +
ac\comp d + ab\comp d + bcd + \comp b \comp c \comp d$
...
Die KMF ist daher $(\comp b + c) \cdot (a + \comp d) \cdot (c + \comp d) \cdot
(a + \comp b + c) $
